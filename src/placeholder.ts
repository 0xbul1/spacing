...
export function createPlaceholderElement(
  ...
  placeholder.classList.add(`spacing-${type}-placeholder`);
  ...
  dimension.innerText = `${arrow} ${Math.round(width)}px x ${Math.round(height)}px`;
  placeholder.appendChild(dimension);
}

export function clearPlaceholderElement(type: PlaceholderType):void {
  document.querySelector(`.spacing-${type}-placeholder`)?.remove();
}